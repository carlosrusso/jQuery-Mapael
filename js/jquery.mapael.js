...
(function ($) {
  ...
  $.fn.mapael = function (options) {
    ...
    return this.each(function () {
      ...
      var mat = paper.rect(0, 0, paper.width, paper.height).attr("fill", "#FFF").attr('stroke', '#000000');
      console.log('paper:', paper.width, paper.height);
      window.paper = paper;
      var box;
      //var set = paper.set();
      paper.setStart();
      ...
      $self.on("zoom", function (e, zoomOptions) {
        ...
        $self.data({
          ...
          "panX": panX,
          "panY": panY,
          "zoomX": panX + paper._viewBox[2] / 2,
          "zoomY": panY + paper._viewBox[3] / 2
        });
      });
...
//when mouse goes down over background, start drawing selection box
      function dragstart(x, y, event) {
        ...
        var fx = (event.clientX - bnds.left) / bnds.width * mat.attrs.width;
        var fy = (event.clientY - bnds.top) / bnds.height * mat.attrs.height;
        box = paper.rect(fx, fy, 0, 0).attr("stroke", "#9999FF");
      ...
      function dragmove(dx, dy, x, y, event) {
        ...
        var zoomLevel = paper.width / mapConf.width;
        box.attr("width", dx / zoomLevel);
        box.attr("height", dy / zoomLevel);
        console.log(event);
      }
      ...
      function dragend(event) {
        ...
        for (var c in set.items) {
          //here, we want to get the x,y vales of each object regardless of what sort of shape it is,
      ...
      function updateSelections() {
        selections.forEach($.fn.mapael.toggleSelection)
      };
  ...
  $.fn.mapael.toggleSelection = function (elem) {
    var newState = !($(elem.node).attr("data-isselected") == "true");
    $(elem.node).attr("data-isselected", newState);
    elem.attr("opacity", newState ? 0.5 : 1);
  }
  ...
  $.fn.mapael.initElem = function (paper, elem, options, $tooltip, id) {
    ...
    $(elem.mapElem.node).attr("data-isselected", false);
  };
  ...
  $.fn.mapael.initZoom = function ($container, paper, mapWidth, mapHeight, options) {
    ...
    $zoomIn.on("click", function () {
      $parentContainer.trigger("zoom", {"level": $parentContainer.data("zoomLevel") + 1});
    });
        ...
        if (Math.abs(offsetX) > 5 || Math.abs(offsetY) > 5) {
          ...
          $.fn.mapael.panningTO = setTimeout(function () {
            $container.trigger("afterPanning", {
              x1: panX,
              y1: panY,
              x2: (panX + paper._viewBox[2]),
              y2: (panY + paper._viewBox[3])
            });
          }, 150);
  ...
  $.fn.mapael.setHover = function (paper, mapElem, textElem) {
    var $mapElem = {}
      ...
      , overBehaviour = function () {
        hoverTO = setTimeout(function () {
          $.fn.mapael.elemHover(paper, mapElem, textElem);
        }, 120);
      }
    ...
    $mapElem.on("click", function(){$.fn.mapael.toggleSelection(mapElem)});
  ...
  $.fn.mapael.animateViewBox = function animateViewBox($container, paper, x, y, w, h, duration, easingFunction) {
    ...
    $.fn.mapael.animationIntervalID = setInterval(function () {
        ...
        if (current_step++ >= steps) {
          ...
          clearTimeout($.fn.mapael.zoomTO);
          $.fn.mapael.zoomTO = setTimeout(function () {
            $container.trigger("afterZoom", {x1: x, y1: y, x2: (x + w), y2: (y + h)});
          }, 150);
        }
      }
      , interval
